...
- **Regra de Ouro**: Todo componente nasce como _Server Component_.
...
- _Exemplo_: A estrutura principal da tabela é renderizada no servidor. Apenas as "células" interativas (botão de favoritar, abrir modal de detalhes) são convertidas para Client Components em folhas (leaves) da árvore.
